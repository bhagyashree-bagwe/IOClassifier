...
def find_typerefs(node,filename):
    ...
    print '%s#%s#%s#%s#%s#%s' % (node.kind,node.displayname if node.displayname else "null1", node.location.line, node.extent.start.line,node.extent.end.line,node.lexical_parent.displayname if node.lexical_parent else "null2")
